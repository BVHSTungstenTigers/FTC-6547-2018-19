...
public class TeleOp_DYLANandHERIOSON extends theColt {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (feildRealtive) {
                if (gameTime.seconds() < 90)
                    lights.setPattern(RevBlinkinLedDriver.BlinkinPattern.CP1_BREATH_SLOW);
                double speed = Math.hypot(gamepad1.left_stick_x, gamepad1.left_stick_y);
                double LeftStickAngle = Math.atan2(-gamepad1.left_stick_y, gamepad1.left_stick_x) - Math.PI / 4;
                if (offset)
                    LeftStickAngle = Math.atan2(-gamepad1.left_stick_y, gamepad1.left_stick_x) - (Math.PI / 4) + Math.toRadians(45);
            ...
            else if (gamepad2.left_stick_y < 0 && hanger.getCurrentPosition() < -7200)
                hanger.setPower(0);
